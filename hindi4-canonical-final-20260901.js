...
// 1) Собираем фактический состав коллективного курса «Роль религий…» до удаления дублей.
const roleReaders=[];
DATA.forEach(r=>{
  if(!isRole(r)) return;
...
// 2) Удаляем только конфликтующие/дублирующие представления целевого блока.
...
function addForReaders(base, readers){
  readers.forEach(t=>{
    DATA.push({...base, teacher:t, joint_readers:readers.filter(x=>x!==t)});
  });
}

// 3) История литературы: один и тот же курс для студентов и для каждого преподавателя,
// преподаватели меняются по распределению недель, а не ведут две параллельные пары.
...
[
  ...
  room_hint:room,
  quality:'01.09.2026, правка №13: восстановлена единая кафедральная схема курса для 4 курса историков-хинди; дубль Стрелкова/Волхонский объединён как последовательное чтение по неделям.'
},litReaders));

// 4) Спецсеминар Никольской — подтверждённый отдельный курс всей группы.
DATA.push({
...
// 5) ВВЯ (синхала): дисциплина реальная, но безопасный второй точный слот не подтверждён.
DATA.push({
...
// 6) Соцпол — один совместный шестипарный курс. Безопасного общего слота в допустимых Вт/Чт нет:
// Вт13 занят западным языком; остальные допустимые часы заняты подтверждённой нагрузкой преподавателей/группы.
...
// 7) «Роль религий…» — единый коллективный курс всего потока, а не набор параллельных карточек.
...
// 8) Ложное попадание группы Сюннерберга 23Б/И412/7 в хинди: фильтруем только в студенческом представлении хинди.
try{
  if(typeof fullGroupMatches==='function' && !fullGroupMatches.__hindi13){
    ...
    wrapped.__hindi13=true;
    fullGroupMatches=wrapped;
  }
}catch(e){}

// 9) Для историков-хинди не считаем «все филологи» реальным конфликтом.
try{
  if(typeof findConflicts==='function' && !findConflicts.__hindi13){
    ...
    wrapped.__hindi13=true;
    findConflicts=wrapped;
  }
}catch(e){}

// 10) Обновляем публичные данные. В базовом интерфейсе строки коллективного чтения
// с одинаковыми extra/day/time автоматически объединяются у студентов, а у каждого преподавателя
// отображается та же пара с тем же временем, группой и пометкой совместного чтения.
